...
</rss>""".format(datetime.now(timezone.utc).strftime('%a, %d %b %Y %H:%M:%S GMT'))
...
def run(server_class=http.server.HTTPServer, handler_class=MyHandler):
    ...
    certfile = config.get('Server', 'certfile', fallback='localhost.crt')
    keyfile = config.get('Server', 'keyfile', fallback='localhost.key')
